...
def test_weight_port_roundtrip_matches_pytorch_reference_small_scale():
    ...
    class TorchEarthAttention3D(nn.Module):
        def __init__(self, dim, heads, type_of_windows):
            ...
            self.earth_specific_bias = nn.Parameter(
                torch.zeros(1, type_of_windows, heads, vol, vol)
            )
    ...
    class TorchEarthSpecificBlock(nn.Module):
        ...
        def forward(self, x):
            ...
            x = F.pad(
                x, (0, 0, 0, 0, self.padding_front, self.padding_back), "constant"
            )
            ori_shape = x.shape
            ...
            x_window = x.view(
                x.shape[0], Z // wZ, wZ, self.H_pad // wH, wH, W // wW, wW, x.shape[-1]
            )
            x_window = x_window.permute(0, 5, 1, 3, 2, 4, 6, 7)
            x_window = x_window.reshape(
                x_window.shape[0] * x_window.shape[1],
                self.type_of_windows,
                wZ,
                wH,
                wW,
            ...
            x_window = x_window.reshape(
                x_window.shape[0], x_window.shape[1], wZ * wH * wW, x_window.shape[-1]
            )

            attn = self.attention(x_window, mask)

            x_shifted = attn.view(
                x.shape[0], W // wW, Z // wZ, self.H_pad // wH, wZ, wH, wW, -1
            )
            ...
            if self.shift:
                x = torch.roll(
                    x_shifted, shifts=[wZ // 2, wH // 2, wW // 2], dims=(1, 2, 3)
                )
            else:
                x = x_shifted
    ...
    for shift in (False, True):
        ...
        state_dict = {
            k: v.detach().numpy() for k, v in torch_block.state_dict().items()
        }

        keras_block = EarthSpecificBlock(
            dim, heads, resolution, shift=shift, name="block"
        )
        x0 = np.zeros(
            (1, resolution[0] * resolution[1] * resolution[2], dim), dtype="float32"
        )
        keras_block(x0)

        mapper = {
            "norm1.weight": "block/norm1/gamma",
            "norm1.bias": "block/norm1/beta",
            "norm2.weight": "block/norm2/gamma",
            "norm2.bias": "block/norm2/beta",
        ...
        report = WeightConverter(
            keras_block, state_dict, lambda k: mapper.get(k)
        ...
        x_np = np.random.randn(
            2, resolution[0] * resolution[1] * resolution[2], dim
        ).astype("float32")
        with torch.no_grad():
...
def test_real_pretrained_pangu_weather_matches_reference_implementation():
    if keras.backend.backend() != "torch":
        pytest.skip(
            "PanguWeather's full-resolution forward pass needs the torch backend - "
    ...
    class EarthAttention3D(nn.Module):
        def __init__(self, dim, heads):
            ...
            vol = window_size[0] * window_size[1] * window_size[2]
            self.earth_specific_bias = nn.Parameter(
                torch.zeros(1, self.type_of_windows, heads, vol, vol)
            )
    ...
    class EarthSpecificBlock(nn.Module):
        ...
        def forward(self, x, Z, H, W, roll):
            ...
            x_shifted = attn_windows.view(
                1, attn_windows.shape[0], Z // wZ, H // wH + 1, wZ, wH, wW, -1
            )
    ...
    class PanguModel(nn.Module):
        def __init__(
            ...
            for i in range(4):
                layer_list[f"EarthSpecificLayer{i}"] = EarthSpecificLayer(
                    depths[i], dims[i], num_heads[i]
                )
    ...
    with torch.no_grad():
        torch_out, torch_out_surface = torch_model(
            torch.from_numpy(raw_upper),
            torch.from_numpy(raw_surface),
            [zero_s, one_s, zero_u, one_u],
            torch.from_numpy(maps),
            torch.from_numpy(const_h),
        )
    ...
    upper_6ch = np.concatenate(
        [np.transpose(raw_upper[0], (1, 2, 3, 0)), const_h[0, 0, 0][:, :, :, None]],
        axis=-1,
    )[None]
    surface_7ch = np.concatenate(
        [np.transpose(raw_surface[0], (1, 2, 0)), np.transpose(maps_721[0], (1, 2, 0))],
        axis=-1,
    )[None]
    ...
    with torch.no_grad():
        keras_out, keras_out_surface = keras_model(
            [upper_6ch, surface_7ch], training=False
        )
    ...
    assert (
        max_diff_upper < 1e-2
    ), f"PanguWeather upper output mismatch: max abs diff {max_diff_upper}"
    assert (
        max_diff_surface < 1e-2
    ), f"PanguWeather surface output mismatch: max abs diff {max_diff_surface}"
