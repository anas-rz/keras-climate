...
def test_builds_and_runs(variant):
    ...
    model = ConvLSTMNowcaster(input_shape=(5, 32, 32, 1), pred_steps=4, out_channels=1, **cfg)
...
def test_decoder_weights_are_shared_across_rollout_steps():
    ...
    for pred_steps in (2, 5, 9):
        model = ConvLSTMNowcaster(input_shape=(3, 16, 16, 1), pred_steps=pred_steps, filters=filters)
        params_by_pred_steps.add(model.count_params())
    assert len(params_by_pred_steps) == 1, (
        f"parameter count changed with pred_steps: {params_by_pred_steps}")


def test_multichannel_output():
    model = ConvLSTMNowcaster(input_shape=(4, 16, 16, 2), pred_steps=3, filters=(8,), out_channels=3)
...
def test_weight_port_roundtrip_matches_pytorch_reference():
    ...
    class ConvLSTMStack(nn.Module):
        def __init__(self, layer_specs, kernel_size=3):
            ...
            self.cell_list = nn.ModuleList([
                ConvLSTMCell(in_dim, hidden_dim, kernel_size) for in_dim, hidden_dim in layer_specs
            ])
    ...
    translated = convert_convlstm_stack_state_dict(flat, layer_specs, keras_prefix="encoder_convlstm")
