...
def test_builds_and_runs(stack_types):
    model = NBeats(seq_len=48, pred_len=12, stack_types=stack_types, num_blocks_per_stack=2,
                    hidden_dim=32, num_fc_layers=2, trend_degree=2, num_harmonics=3)
...
def test_weight_port_roundtrip_matches_pytorch_reference():
    ...
    class TorchTrendBasis(nn.Module):
        def __init__(self, degree, backcast_size, forecast_size):
            ...
            self.degree = degree
            poly = degree + 1
            self.register_buffer("backcast_basis", torch.from_numpy(
                np.stack([(np.arange(backcast_size) / backcast_size) ** i for i in range(poly)]).astype("float32")))
            self.register_buffer("forecast_basis", torch.from_numpy(
                np.stack([(np.arange(forecast_size) / forecast_size) ** i for i in range(poly)]).astype("float32")))

        def forward(self, theta):
            poly = self.degree + 1
            forecast_theta, backcast_theta = theta[:, :poly], theta[:, poly:]
            return backcast_theta @ self.backcast_basis, forecast_theta @ self.forecast_basis
    ...
    class TorchSeasonalityBasis(nn.Module):
        def __init__(self, num_harmonics, backcast_size, forecast_size):
            ...
            self.register_buffer("backcast_cos", torch.from_numpy(np.cos(2 * np.pi * k * t_back).astype("float32")))
            self.register_buffer("backcast_sin", torch.from_numpy(np.sin(2 * np.pi * k * t_back).astype("float32")))
            self.register_buffer("forecast_cos", torch.from_numpy(np.cos(2 * np.pi * k * t_fore).astype("float32")))
            self.register_buffer("forecast_sin", torch.from_numpy(np.sin(2 * np.pi * k * t_fore).astype("float32")))
    ...
    class TorchNBeats(nn.Module):
        def __init__(self, seq_len, pred_len, stack_types, num_blocks_per_stack, hidden_dim,
                     num_fc_layers, trend_degree, num_harmonics):
            super().__init__()
            blocks = []
            for stack_type in stack_types:
                for _ in range(num_blocks_per_stack):
                    basis, theta_size = make_basis(stack_type, seq_len, pred_len, trend_degree, num_harmonics)
                    blocks.append(TorchBlock(seq_len, hidden_dim, num_fc_layers, basis, theta_size))
    ...
    torch_model = TorchNBeats(seq_len, pred_len, stack_types, num_blocks_per_stack, hidden_dim,
                               num_fc_layers, trend_degree, num_harmonics)
    torch_model.eval()
    with torch.no_grad():
    ...
    keras_model = NBeats(seq_len=seq_len, pred_len=pred_len, stack_types=stack_types,
                          num_blocks_per_stack=num_blocks_per_stack, hidden_dim=hidden_dim,
                          num_fc_layers=num_fc_layers, trend_degree=trend_degree,
    ...
    assert max_diff < 1e-2, f"N-BEATS weight port numerical mismatch: max abs diff {max_diff}"
