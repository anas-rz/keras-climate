...
def test_weight_port_roundtrip_matches_pytorch_reference():
    ...
    class TorchDistillConv(nn.Module):
        def __init__(self, d_model):
            super().__init__()
            self.conv = nn.Conv1d(
                d_model, d_model, kernel_size=3, padding=1, bias=False
            )
    ...
    class TorchInformer(nn.Module):
        def __init__(
            ...
            self.encoder_layers = nn.ModuleList(
                [
                    TorchEncoderLayer(d_model, num_heads, d_ff)
                    for _ in range(encoder_layers)
                ]
            )
            self.distills = (
                nn.ModuleList(
                    [TorchDistillConv(d_model) for _ in range(encoder_layers - 1)]
                )
                if distil
                else None
            )
            self.decoder_layers = nn.ModuleList(
                [
                    TorchDecoderLayer(d_model, num_heads, d_ff)
            ...
            mask = torch.triu(
                torch.ones(total_dec_len, total_dec_len) * -1e9, diagonal=1
            )
    ...
    for k, v in torch_model.state_dict().items():
        if (
            ...
            or k.endswith("position_embedding")
            or k.endswith("self_mask")
        ):
            continue
    ...
    assert set(report["missing_in_source"]) == {
        "enc_embedding/position_embedding",
        "dec_embedding/position_embedding",
    }
    ...
    with torch.no_grad():
        torch_out = torch_model(
            torch.from_numpy(enc_np), torch.from_numpy(dec_np)
        ).numpy()

    keras_out = keras.ops.convert_to_numpy(
        keras_model([enc_np, dec_np], training=False)
    )

    max_diff = np.abs(torch_out - keras_out).max()
    assert (
        max_diff < 1e-2
    ), f"Informer weight port numerical mismatch: max abs diff {max_diff}"
