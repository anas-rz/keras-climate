...
from keras_climate.remote_sensing.deeplabv3plus import (
    DeepLabV3Plus,
...
def test_builds_and_runs(variant):
    ...
    model = DeepLabV3Plus(
        input_shape=(128, 128, 3), num_classes=5, output_stride=16, **cfg
    )
...
def test_non_multiple_of_32_input_size():
    model = DeepLabV3Plus(
        input_shape=(130, 130, 3), num_classes=2, backbone_layers=(3, 4, 6, 3)
    )
...
def test_weight_port_roundtrip_matches_pytorch_reference():
    ...
    class ResNetBackbone(nn.Module):
        def __init__(self, layer_counts=(3, 4, 6, 3), output_stride=16):
            ...
            self.layer1 = self._make_layer(
                64, layer_counts[0], strides[0], dilations[0]
            )
            self.layer2 = self._make_layer(
                128, layer_counts[1], strides[1], dilations[1]
            )
            self.layer3 = self._make_layer(
                256, layer_counts[2], strides[2], dilations[2]
            )
            self.layer4 = self._make_layer(
                512, layer_counts[3], strides[3], dilations[3]
            )
        ...
        def _make_layer(self, filters, num_blocks, stride, dilation):
            ...
            for _ in range(1, num_blocks):
                layers_.append(
                    Bottleneck(self.in_ch, filters, stride=1, dilation=dilation)
                )
    ...
    class TorchDeepLabV3Plus(nn.Module):
        ...
        def forward(self, x):
            ...
            feat = self.aspp(feat)
            feat = F.interpolate(
                feat, size=low_level.shape[2:], mode="bilinear", align_corners=False
            )
    ...
    torch.manual_seed(0)
    layer_counts = (3, 4, 6, 3)
    torch_model = TorchDeepLabV3Plus(
        num_classes=5, layer_counts=layer_counts, output_stride=16
    )
    torch_model.eval()
    with torch.no_grad():
    ...
    keras_model = DeepLabV3Plus(
        input_shape=(128, 128, 3),
        num_classes=5,
        backbone_layers=layer_counts,
        output_stride=16,
    )
    ...
    assert (
        max_diff < 1e-2
    ), f"DeepLabV3+ weight port numerical mismatch: max abs diff {max_diff}"
...
def test_real_pretrained_imagenet_resnet50_backbone():
    ...
    model, report = deeplabv3plus_resnet50_imagenet_backbone(
        input_shape=(224, 224, 3), num_classes=21
    )
    assert all(
        k.startswith(("aspp/", "low_level_project/", "decoder_", "logits/"))
