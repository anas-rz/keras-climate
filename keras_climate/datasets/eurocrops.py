"""EuroCrops dataset (ported from torchgeo.datasets.eurocrops)."""
...
class EuroCrops(VectorDataset):
    ...
    The `EuroCrops <https://www.eurocrops.tum.de/index.html>`__ dataset
    combines "all publicly available self-declared crop reporting datasets
    from countries of the European Union" into a unified format. The dataset
    is released under CC BY 4.0 Deed.

    The dataset consists of shapefiles containing a total of 22M polygons.
    Each polygon is tagged with an "EC_hcat_n" attribute indicating the
    harmonized crop name grown within the polygon in the year associated
    with the shapefile.

    If you use this dataset in your research, please follow the citation
    guidelines at:
    ...
    * https://github.com/maja601/EuroCrops#reference.
    ...
    def __init__(
        ...
        Args:
            ...
            crs: CRS to warp to (defaults to WGS-84)
            res: resolution of the dataset in units of CRS (xres, yres) format
            classes: list of classes to include (specified by their HCAT
                code), the rest will be mapped to 0 (defaults to all classes)
        ...
        super().__init__(
            paths=paths, crs=crs, res=res, transforms=transforms, label_name=self.label_name
        )
    ...
    def _load_class_map(self, classes):
        ...
        If classes is provided, then we simply use those codes. Otherwise,
        we load all the codes from the CSV file.
        """
        if not classes:
            assert isinstance(self.paths, (str, os.PathLike))
            classes = []
    ...
    def get_label(self, feature):
        ...
        Converts the HCAT code of this feature to its index per
        ``self.class_map``. We go up the class hierarchy until there is a
        match (parent code is computed by replacing rightmost non-0
        character with 0).
        """
    ...
    def plot(self, sample, show_titles=True, suptitle=None):
        ...
        if showing_prediction:
            pred = ops.convert_to_numpy(sample["prediction"]).squeeze()
            ncols = 2

        fig, axs = plt.subplots(nrows=1, ncols=ncols, figsize=(4, 4))

        def apply_cmap(array):
            """Color 0 as black, applying the default colormap to class indices."""
