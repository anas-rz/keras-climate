"""Open Buildings datasets (ported from torchgeo.datasets.openbuildings)."""
...
class OpenBuildings(VectorDataset):
    ...
    The `Open Buildings <https://sites.research.google/gr/open-buildings/>`__
    dataset consists of computer generated building detections across the
    African continent.
    ...
    * confidence score and `Plus Code <https://maps.google.com/pluscodes/>`_
    ...
    * https://arxiv.org/abs/2107.12283
    ...
    def __init__(self, paths="data", crs=None, res=0.0001, transforms=None, checksum=True):
        ...
        filename = os.path.join(paths, self.meta_data_filename)
        gdf = gpd.read_file(filename)
        if gdf.crs is None:
            gdf = gdf.set_crs(self._source_crs)
    ...
    def __getitem__(self, index):
        ...
        if shapes:
            ...
            masks = np.zeros((round(height), round(width), 1), dtype="uint8")

        transform = rasterio.transform.from_origin(x.start, y.stop, x.step, y.step)
        sample = {
            "mask": ops.convert_to_tensor(masks),
    ...
    def _filter_geometries(self, index, filepaths):
        """Filters buildings read from the polygon csv files based on index."""
    ...
    def plot(self, sample, show_titles=True, suptitle=None):
        ...
        axs_list[0].imshow(mask)
        axs_list[0].axis("off")
        if show_titles:
            axs_list[0].set_title("Mask")

        if showing_predictions:
            axs_list[1].imshow(pred)
            axs_list[1].axis("off")
            if show_titles:
