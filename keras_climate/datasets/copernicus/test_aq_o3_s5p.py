...
from keras_climate.datasets.copernicus.aq_o3_s5p import CopernicusBenchAQO3S5P
...
def _prepare_root(tmp_path, split="train"):
    root = str(tmp_path)
    directory = os.path.join(root, "airquality_s5p", "o3")
    pid = "loc_0001"
...
def test_getitem_annual(tmp_path):
    ...
    ds = CopernicusBenchAQO3S5P(root=root, split="train", mode="annual")
    sample = ds[0]
...
def test_getitem_seasonal(tmp_path):
    root = _prepare_root(tmp_path)
    ds = CopernicusBenchAQO3S5P(root=root, split="train", mode="seasonal")
    sample = ds[0]
...
def test_len(tmp_path):
    root = _prepare_root(tmp_path)
    ds = CopernicusBenchAQO3S5P(root=root, split="train")
    assert len(ds) == 1


def test_not_downloaded(tmp_path):
    with pytest.raises(DatasetNotFoundError):
        CopernicusBenchAQO3S5P(root=str(tmp_path), download=False)
