...
from keras_climate.datasets.copernicus.dfc2020_s2 import CopernicusBenchDFC2020S2

TRANSFORM = from_origin(10, 50, 0.01, 0.01)
N_BANDS = len(CopernicusBenchDFC2020S2.all_bands)


def _write_image(path, bands, size=8, dtype="uint16"):
    os.makedirs(os.path.dirname(path), exist_ok=True)
    data = np.random.randint(0, 4000, (bands, size, size)).astype(dtype)
    with rasterio.open(
        path,
        "w",
        driver="GTiff",
...
def _prepare_root(tmp_path, split="train"):
    ...
    s2_file = "s2_0001.tif"
    _write_image(os.path.join(directory, "s2", s2_file), bands=N_BANDS)
    _write_mask(
        os.path.join(directory, "dfc", dfc_file),
        n_classes=len(CopernicusBenchDFC2020S2.classes),
    )
...
def test_getitem(tmp_path):
    ...
    ds = CopernicusBenchDFC2020S2(root=root, split="train")
    sample = ds[0]
...
def test_not_downloaded(tmp_path):
    with pytest.raises(DatasetNotFoundError):
        CopernicusBenchDFC2020S2(root=str(tmp_path), download=False)
