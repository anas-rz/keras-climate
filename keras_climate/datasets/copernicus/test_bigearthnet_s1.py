...
from keras_climate.datasets.copernicus.bigearthnet_s1 import CopernicusBenchBigEarthNetS1

TRANSFORM = from_origin(10, 50, 0.01, 0.01)
N_CLASSES = len(CopernicusBenchBigEarthNetS1.classes)
...
def _prepare_root(tmp_path, split="train"):
    ...
    _write_tif(os.path.join(directory, "BigEarthNet-S2-5%", s2_name), bands=12)
...
def test_getitem(tmp_path):
    ...
    ds = CopernicusBenchBigEarthNetS1(root=root, split="train")
    sample = ds[0]
...
def test_not_downloaded(tmp_path):
    with pytest.raises(DatasetNotFoundError):
        CopernicusBenchBigEarthNetS1(root=str(tmp_path), download=False)
