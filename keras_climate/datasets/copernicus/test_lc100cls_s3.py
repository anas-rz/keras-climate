...
from keras_climate.datasets.copernicus.lc100cls_s3 import CopernicusBenchLC100ClsS3
...
def test_len(prepared_root):
    ds = CopernicusBenchLC100ClsS3(root=prepared_root, split="train", download=False)
    assert len(ds) == 1


def test_not_downloaded(tmp_path):
    with pytest.raises(DatasetNotFoundError):
        CopernicusBenchLC100ClsS3(root=str(tmp_path), download=False)
