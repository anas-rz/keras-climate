"""SeasoNet dataset (ported from torchgeo.datasets.seasonet)."""
...
class SeasoNet(NonGeoDataset):
    ...
    * https://doi.org/10.1109/IGARSS46834.2022.9884079
    ...
    def __init__(
        ...
        Args:
            ...
            concat_seasons: number of seasonal images to return per sample.
                ...
                specified in ``seasons`` and returned stacked along the
                channel axis
    ...
    def __getitem__(self, index):
        ...
        Returns:
            sample containing the image with shape T x H x W x C and the mask
            with shape H x W, where ``T = self.concat_seasons``
        """
    ...
    def _load_image(self, index):
        ...
        Returns:
            the stacked seasonal images, shape (T, H, W, C)
        """
        paths = self.files.iloc[index]
        if self.concat_seasons > 1:
            paths = random.sample(paths, self.concat_seasons)
        array = np.zeros(
            (self.concat_seasons, *self.image_size, self.channels), dtype="float32"
        )
        for img_idx, path in enumerate(paths):
            ...
            for band in self.bands:
                ...
                # band_data is (band_count, H, W) -> (H, W, band_count)
                band_data = np.transpose(band_data, (1, 2, 0)).astype("float32")
                array[img_idx, :, :, bnd_idx : bnd_idx + band_data.shape[-1]] = (
                    band_data
                )
                bnd_idx += band_data.shape[-1]
    ...
    def plot(self, sample, show_titles=True, suptitle=None, show_legend=True):
