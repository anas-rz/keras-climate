"""BioMassters Dataset (ported from torchgeo.datasets.biomassters)."""
...
class BioMassters(NonGeoDataset):
    ...
    Dataset intended for Aboveground Biomass (AGB) prediction over Finnish
    forests based on Sentinel 1 and 2 data with corresponding target AGB
    mask values generated by Light Detection and Ranging (LiDAR).
    ...
    * Sentinel 2 available for almost every month (not available for every
      month due to ESA acquisition halt over the region during particular
      periods)
    ...
    * https://nascetti-a.github.io/BioMasster/
    ...
    .. note::
        ...
        <https://huggingface.co/datasets/torchgeo/biomassters>`_.
    """
    ...
    def __init__(
        self, root="data", split="train", sensors=["S1", "S2"], as_time_series=False
    ):
        """Initialize a new instance of BioMassters dataset.

        If ``as_time_series=False`` (the default), each time step becomes
        its own sample with the target being shared across multiple
        samples.

        Args:
            root: root directory where dataset can be found
            split: train or test split
            sensors: which sensors to consider for the sample, Sentinel 1
                and/or Sentinel 2 ('S1', 'S2')
        ...
        # generate numerical month from filename since first month is
        # September and has numerical index of 0
        self.df["num_month"] = (
            self.df["filename"]
    ...
    def _load_input(self, filenames):
        """Load the input imagery at the index (channels-last)."""
        filepaths = [
        ...
        for fp in filepaths:
            with rasterio.open(fp) as src:
                # (C, H, W) -> (H, W, C)
                arr_list.append(np.transpose(src.read(), (1, 2, 0)))
        if self.as_time_series:
            arr = np.stack(arr_list, axis=0)
        else:
            arr = np.concatenate(arr_list, axis=-1)
        return ops.convert_to_tensor(arr.astype("int32"))

    def _load_target(self, filename):
        """Load the target mask at the index (channels-last)."""
        with rasterio.open(os.path.join(self.root, "train_agbm", filename), "r") as src:
            arr = np.transpose(src.read(), (1, 2, 0))

        return ops.cast(ops.convert_to_tensor(arr), "float32")
