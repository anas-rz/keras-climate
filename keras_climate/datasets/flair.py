"""FLAIR (French Land cover from Aerospace ImageRy) datasets (ported from torchgeo.datasets.flair)."""
...
class FLAIRHUBBase(NonGeoDataset):
    ...
    `FLAIR-HUB <https://github.com/IGNF/FLAIR-HUB>`__ builds upon and includes
    the FLAIR#1 and FLAIR#2 datasets, expanding them into a unified,
    large-scale, multi-sensor land-cover resource with very-high-resolution
    annotations. Spanning over 2,500 km^2 of diverse French ecoclimatic
    regions and landscapes, FLAIR-HUB features 63 billion hand-annotated
    pixels across 19 land-cover and 23 crop type classes.
    ...
    The dataset is organized by domains (geographical areas) and years. Each
    domain-year combination has multiple modalities available for download.

    Available modalities (100% coverage across all domains):

    - ``AERIAL_RGBI``: High-resolution aerial imagery (RGB + NIR, 0.2m) --
      key: ``image_aerial_rgbi``
    - ``SPOT_RGBI``: SPOT satellite imagery (RGB + NIR, 1.5m) -- key:
      ``image_spot_rgbi``
    - ``DEM_ELEV``: Digital Elevation Model (DSM + DTM, 1m) -- key:
      ``image_dem_elev``
    - ``AERIAL-RLT_PAN``: Historical aerial panchromatic (1950s) -- key:
      ``image_aerial_rlt_pan``
    - ``SENTINEL1-ASC_TS``: Sentinel-1 SAR Ascending time series (VV + VH) --
      key: ``image_sentinel1_asc``
    - ``SENTINEL1-DESC_TS``: Sentinel-1 SAR Descending time series (VV + VH)
      -- key: ``image_sentinel1_desc``
    - ``SENTINEL2_TS``: Sentinel-2 multispectral time series (12 bands, 10m)
      -- key: ``image_sentinel2``
    - ``SENTINEL2_MSK-SC``: Sentinel-2 scene classification mask -- key:
      ``mask_sentinel2_snowcloud``
    ...
    def __init__(
        ...
        The FLAIR-HUB dataset provides multiple complementary data modalities
        for land cover and crop type mapping. You can selectively load any
        combination of the available modalities by specifying them in the
        *bands* parameter.

        Only samples belonging to the specified split (train, val, or test)
        are loaded, using the official splits from
        ``GLOBAL_ALL_MTD_SPLIT.gpkg``.

        Args:
            root: Root directory where dataset can be found or will be
                downloaded.
            split: One of "train", "val", or "test".
            split_column: Column name in the official splits GeoPackage
                (e.g. "split_1", "split_5", "split_flairchallenge").
            transforms: Optional transforms to apply to samples.
            download: If True, download the dataset if it is not found.
            checksum: If True, check the SHA-256 of downloaded files (may be
                slow).
            bands: List of bands/modalities to load. Defaults to None, which
                enables all bands. See the modality key map above for
                available options.
            dataset_type: Type of labels to use. Choose "land_cover" for
                19-class COSIA labels or "crop_type" for 23-class LPIS crop
                classification labels (baseline) or "crop_type_2" for
                31-class LPIS crop classification labels or "crop_type_3"
                for 46-class LPIS crop classification labels.
    ...
    def _ensure_splits_available(self):
        ...
        Raises:
            DatasetNotFoundError: If the splits file is missing and
                *download* is False.
        """
        ...
        if zip_path.exists():
            if self.checksum and not check_integrity(zip_path, sha256=self.sha256s[zip_path.name]):
                raise RuntimeError("Dataset found, but corrupted.")
        elif self.download:
            sha256 = self.sha256s[zip_path.name] if self.checksum else None
            download_url(f"{self.download_link}/GLOBAL_ALL_MTD.zip", str(root), sha256=sha256)
        else:
    ...
    def __getitem__(self, index):
        ...
        Returns a dictionary containing tensors for each modality. Keys are
        the modality names prefixed with ``image_`` or ``mask_``: ``mask``,
        ...
        for modality_name in self.bands:
            ...
            if "TS" in modality_name or "SC" in modality_name:
                if modality_name == "SENTINEL2_TS":
                    band_names = self.sentinel2_ts_bands
                elif modality_name == "SENTINEL2_MSK-SC":
                    band_names = self.sentinel2_msk_sc_bands
                elif modality_name == "SENTINEL1-ASC_TS":
                    band_names = self.sentinel1_asc_ts_bands
                elif modality_name == "SENTINEL1-DESC_TS":
                    band_names = self.sentinel1_desc_ts_bands

                sample[sample_key] = self._load_time_series(modality_path, len(band_names))
            else:
    ...
    def _load_files(self):
        ...
        allowed_patch_ids = set(gdf.loc[gdf[self.split_column] == gpkg_split, "patch_id"].astype(str))
        ...
        for label_path in pathlib.Path(self.root).glob(filename_glob):
            ...
            # Add modality
            for modality_name in self.bands:
                file_path = pathlib.Path(str(label_path).replace(label_dir, modality_name))
                if modality_name == "AERIAL-RLT_PAN":
                    ...
                    new_file_path = pathlib.Path(str(file_path).replace(region_year, new_region_year))
    ...
    def _verify(self):
        ...
        label_modality = "AERIAL_LABEL-COSIA" if self.dataset_type == "land_cover" else "ALL_LABEL-LPIS"
            ...
            for year in years:
                for modality in [label_modality, *self.bands]:
                    ...
                    if zip_path.is_file():
                        ...
                        sha256 = self.sha256s[zip_path.name] if self.checksum else None
                        download_url(f"{self.download_link}/{dir_name}.zip", str(self.root), sha256=sha256)
                    else:
                        raise DatasetNotFoundError(self)
    ...
    def _load_mask(self, path):
        ...
        For ``crop_type_2``/``crop_type_3``, the requested band selects the
        appropriate hierarchical granularity level stored in the raster
        (band 2 or 3 respectively); the mask is otherwise single-band.
        """
        if self.dataset_type == "crop_type_3":
            band = 3
        elif self.dataset_type == "crop_type_2":
            band = 2
        else:
            band = 1
    ...
    def _load_time_series(self, path, num_bands):
        """Load a time series from a path.

        Process it to be in the ``(T, H, W, C)`` format instead of the usual
        ``(T*C, H, W)`` format.
        """
        with rasterio.open(str(path)) as f:
            array = f.read()

        t = array.shape[0] // num_bands
        array = array.reshape(t, num_bands, array.shape[1], array.shape[2])
        array = np.transpose(array, (0, 2, 3, 1))
        return ops.cast(array_to_tensor(array), "float32")
    ...
    def _plot_mask(self, mask, ax, show_legend=True, show_titles=True):
    ...
    def _plot_aerial_rgbi(self, data, ax, title, show_titles=True):
        """Plot aerial RGBI imagery."""
        data_np = ops.convert_to_numpy(data)
        rgb_image = data_np[..., :3] / 255.0
        rgb_image = np.clip(rgb_image, 0, 1)
        ax.imshow(rgb_image)
        if show_titles:
            ax.set_title(title)

    def _plot_spot_rgbi(self, data, ax, title, show_titles=True):
        """Plot SPOT RGBI imagery (surface reflectance)."""
        image = quantile_normalization(data[..., :3])
        ax.imshow(ops.convert_to_numpy(image))
        if show_titles:
            ax.set_title(title)

    def _plot_dem(self, data, ax, title, show_titles=True):
    ...
    def _plot_aerial_rlt_pan(self, data, ax, title, show_titles=True):
    ...
    def _plot_sentinel2_ts(self, data, ax, title, show_titles=True):
        """Plot the last timepoint of Sentinel-2 time series data as RGB (B04, B03, B02)."""
        data_np = ops.convert_to_numpy(data)

        # The format of the data is T, H, W, C
        last_timepoint = data_np[-1]

        rgb_indices = [self.sentinel2_ts_bands.index(band) for band in self.sentinel2_ts_rgb_bands]
        rgb_image = last_timepoint[..., rgb_indices]
    ...
    def _plot_sentinel2_msk_sc(self, data, ax, title, show_titles=True):
        ...
        rgb_img = np.zeros((*snow_probability.shape[:2], 3), dtype=np.float32)
        ...
        legend_elements = [
            Patch(facecolor=color, edgecolor="k", label=label) for idx, label, color in present
        ]
    ...
    def _plot_sentinel1_ts(self, data, ax, title, show_titles=True):
        """Plot the last timepoint of Sentinel-1 time series data (VV band, grayscale)."""
        # T, H, W, C -> take last timepoint, VV band (index 0)
        vv = quantile_normalization(data[-1, ..., 0])
        ax.imshow(ops.convert_to_numpy(vv), cmap="gray")
        if show_titles:
            ax.set_title(title)
    ...
    def plot(self, sample, show_titles=True, suptitle=None):
        ...
        plot_data["mask"] = {"plot_type": "mask", "data": sample["mask"], "title": "mask"}

        for key, (plot_type, title) in _PLOT_KEYS.items():
            if key in sample:
                plot_data[key] = {"plot_type": plot_type, "data": sample[key], "title": title}
...
class FLAIRHUBToy(FLAIRHUBBase):
    ...
    For further information refer to :class:`FLAIRHUB`.
    """
    ...
    def __init__(
        ...
        This is a toy/sample version of the FLAIR-HUB dataset intended for
        testing and development purposes. It contains a small subset of the
        full dataset with all available data modalities.

        See :class:`FLAIRHUB` for a description of the arguments.
