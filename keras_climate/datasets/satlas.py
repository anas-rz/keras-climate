"""SatlasPretrain dataset (ported from torchgeo.datasets.satlas)."""
...
from .utils import check_integrity, extract_archive, which
...
class SatlasPretrain(NonGeoDataset):
    ...
    `SatlasPretrain <https://satlas-pretrain.allen.ai/>`__ is a large-scale
    pre-training dataset for tasks that involve understanding satellite
    images. Regularly-updated satellite data is publicly available for much
    of the Earth through sources such as Sentinel-2 and NAIP, and can inform
    numerous applications from tackling illegal deforestation to monitoring
    marine infrastructure. By combining over 30 TB of satellite images with
    137 label categories, SatlasPretrain serves as an effective
    pre-training dataset that greatly reduces the effort needed to develop
    robust models for downstream satellite image applications.
    ...
    * https://doi.org/10.48550/arXiv.2211.15660
    ...
    .. note::
       ...
       * `AWS CLI <https://aws.amazon.com/cli/>`_: to download the dataset
         from AWS.
    """
    ...
    def __init__(
        ...
        root="data",
        split="train_lowres",
        good_images="good_images_lowres_all",
        image_times="image_times",
        images=("sentinel1", "sentinel2", "landsat"),
        labels=("land_cover",),
        transforms=None,
        download=False,
        checksum=True,
    ...
    def _load_image(self, sample, image, col, row, directories):
        """Load a single image and populate *sample* in place."""
        good_directories = []
        ...
        idx = int(np.random.randint(len(good_directories)))
        directory = good_directories[idx]
        time = self.image_times[directory].timestamp()
        sample[f"time_{image}"] = ops.convert_to_tensor(np.array(time, dtype="float64"))
        ...
        for band in self.bands[image]:
            ...
            with Image.open(path) as f:
                ...
                array = np.atleast_3d(np.array(img, dtype="float32"))
                channels.append(array)
        raster = np.concatenate(channels, axis=-1)
        sample[f"image_{image}"] = ops.convert_to_tensor(raster)

    def _load_label(self, sample, label, col, row):
        """Load a single label and populate *sample* in place."""
        path = os.path.join(self.root, "static", f"{col}_{row}", f"{label}.png")
        if os.path.isfile(path):
            with Image.open(path) as img:
                raster = np.array(img, dtype="int64")
        else:
            raster = np.zeros((self.chip_size, self.chip_size), dtype="int64")
        sample[f"mask_{label}"] = ops.convert_to_tensor(raster)
    ...
    def plot(self, sample, show_titles=True, suptitle=None):
        ...
        for key, value in sample.items():
            value = ops.convert_to_numpy(value)
            parts = key.split("_", 1)
            match parts:
                case ["image", "landsat"]:
                    images.append(np.take(value, [3, 2, 1], axis=-1) / 255)
                    titles.append("Landsat 8/9")
                case ["image", "naip"]:
                    images.append(value[..., :3] / 255)
                    titles.append("NAIP")
                ...
                case [("mask" | "prediction"), label]:
                    cmap = np.array(TASKS[label]["colors"])
