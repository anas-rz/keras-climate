"""EnMAP dataset (ported from torchgeo.datasets.enmap)."""
...
class EnMAP(RasterDataset):
    ...
    The Environmental Mapping and Analysis Program (EnMAP) is a German
    hyperspectral satellite mission that monitors and characterizes Earth's
    environment on a global scale. EnMAP measures geochemical, biochemical
    and biophysical variables providing information on the status and
    evolution of terrestrial and aquatic ecosystems.

    Mission Outline:

    * Dedicated pushbroom hyperspectral imager
    * Broad spectral range from 420 nm to 1000 nm (VNIR) and from 900 nm to
      2450 nm (SWIR) with high radiometric resolution and stability
    * 30 km swath width at a spatial resolution of 30 x 30 m, nadir revisit
      time of 27 days and off-nadir (30°) pointing feature for fast target
      revisit (4 days)
    ...
    * https://doi.org/10.1016/j.rse.2023.113632
    ...
    def __init__(
        ...
        Args:
            ...
            bands: bands to return (defaults to ``default_bands``, which
                excludes bands strongly affected by water vapor absorption)
