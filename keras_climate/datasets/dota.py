"""DOTA dataset (ported from torchgeo.datasets.dota)."""
...
class DOTA(NonGeoDataset):
    ...
    `DOTA <https://captain-whu.github.io/DOTA/index.html>`__ is a
    large-scale object detection dataset for aerial imagery containing RGB
    and gray-scale imagery from Google Earth, GF-2 and JL-1 satellites as
    well as additional aerial imagery from CycloMedia. There are three
    versions of the dataset: v1.0, v1.5, and v2.0, where v1.0 and v1.5 have
    the same images but different annotations, and v2.0 extends both the
    images and annotations with more samples.

    Dataset features:

    * 1869 samples in v1.0 and v1.5 and 2423 samples in v2.0
    * multi-class object detection (15 classes in v1.0 and v1.5 and 18
      classes in v2.0)
    ...
    * https://arxiv.org/abs/1711.10398
    ...
    def __init__(
        ...
        Args:
            ...
            bbox_orientation: bounding box orientation, one of
                ['horizontal', 'oriented'], where horizontal returns xyxy
                format and oriented returns x1y1x2y2x3y3x4y4 format
        ...
        Raises:
            AssertionError: if *split*, *version*, or *bbox_orientation*
                argument are not valid
            DatasetNotFoundError: if dataset is not found or corrupted, and
                *download* is False
        """
        assert split in self.valid_splits, f"Split '{split}' not supported, use one of {self.valid_splits}"
        assert version in self.valid_versions, f"Version '{version}' not supported, use one of {self.valid_versions}"
        ...
        self.sample_df = self.sample_df[self.sample_df["version"] == self.version].reset_index(drop=True)
    ...
    def _load_image(self, path):
        """Load image, returned channels-last (H, W, C)."""
        image = Image.open(os.path.join(self.root, path)).convert("RGB")
        return ops.convert_to_tensor(np.array(image).astype("float32"))
    ...
    def _load_annotations(self, path):
        ...
        if not boxes:
            return (
                ops.convert_to_tensor(np.zeros((0, 4 if self.bbox_orientation == "horizontal" else 8), dtype="float32")),
                ops.convert_to_tensor(np.zeros((0,), dtype="int64")),
            )
        else:
            return (
                ops.convert_to_tensor(np.array(boxes, dtype="float32")),
                ops.convert_to_tensor(np.array(labels, dtype="int64")),
            )
    ...
    def _verify(self):
        ...
        required_dirs = [
            ...
            os.path.join(self.root, self.split, "annotations", f"version{self.version}"),
    ...
    def _download(self, files_needed):
        ...
        if not os.path.exists(os.path.join(self.root, self.sample_df_path)):
            download_url(url=self.url.format(self.sample_df_path), root=self.root, filename=self.sample_df_path)

    def _extract(self, files_needed):
        """Extract the dataset."""
