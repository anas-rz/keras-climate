"""COWC datasets (ported from torchgeo.datasets.cowc)."""
...
class COWC(NonGeoDataset, abc.ABC):
    ...
    * https://doi.org/10.1007/978-3-319-46487-9_48
    ...
    def _load_target(self, index):
        ...
        return ops.convert_to_tensor(np.asarray(target, dtype="float32"))
    ...
    def plot(self, sample, show_titles=True, suptitle=None):
        ...
        label = str(float(ops.convert_to_numpy(sample["label"])))

        showing_predictions = "prediction" in sample
        if showing_predictions:
            prediction = str(float(ops.convert_to_numpy(sample["prediction"])))
        else:
            prediction = None
...
class COWCCounting(COWC):
    ...
    base_url = "https://gdo152.llnl.gov/cowc/download/cowc/datasets/patch_sets/counting/"
    filenames = (
...
class COWCDetection(COWC):
    ...
    base_url = "https://gdo152.llnl.gov/cowc/download/cowc/datasets/patch_sets/detection/"
    filenames = (
