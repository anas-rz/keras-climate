"""GlobBiomass dataset (ported from torchgeo.datasets.globbiomass)."""
...
from .utils import check_integrity, disambiguate_timestamp, extract_archive


class GlobBiomass(RasterDataset):
    """GlobBiomass dataset.

    The `GlobBiomass <https://doi.pangaea.de/10.1594/PANGAEA.894711>`__
    dataset consists of global pixelwise aboveground biomass (AGB) and
    growth stock volume (GSV) maps.

    Definitions:

    * AGB: the mass, expressed as oven-dry weight of the woody parts (stem,
      bark, branches and twigs) of all living trees excluding stump and
      roots.
    * GSV: volume of all living trees more than 10 cm in diameter at breast
      height measured over bark from ground or stump height to a top stem
      diameter of 0 cm.
    ...
    * https://doi.org/10.1594/PANGAEA.894711
    ...
    is_image = False
    dtype = "float32"  # pixelwise regression
    ...
    def __getitem__(self, index):
        ...
        mask = ops.concatenate((mask, std_err_mask), axis=-1)
    ...
    def plot(self, sample, show_titles=True, suptitle=None):
        ...
        tensor = ops.convert_to_numpy(sample["mask"])
        mask = tensor[..., 0]
        error_mask = tensor[..., 1]

        showing_predictions = "prediction" in sample
        if showing_predictions:
            pred = ops.convert_to_numpy(sample["prediction"])[..., 0]
            ncols = 3
        else:
            ncols = 2
