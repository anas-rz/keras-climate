"""CDL dataset (ported from torchgeo.datasets.cdl)."""
...
class CDL(RasterDataset):
    ...
    boundary, water and road layers. The data is created annually using moderate
    ...
    * https://www.nass.usda.gov/Research_and_Science/Cropland/sarsfaqs2.php#what.1
    ...
    def __init__(
        ...
        assert set(years) <= self.md5s.keys(), (
            f"CDL data product only exists for the following years: {list(self.md5s.keys())}."
        )
    ...
    def _download(self):
        ...
        for year in self.years:
            download_url(
                self.url.format(year), paths, md5=self.md5s[year] if self.checksum else None
            )
