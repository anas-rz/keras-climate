...
from keras_climate.datasets.spacenet.spacenet5 import SpaceNet5
...
def _build_root(tmp_path, split, num=2):
    aoi_root = os.path.join(str(tmp_path), "SN5_roads", split, f"AOI_{AOI}_Moscow")
    image_dir = os.path.join(aoi_root, "MS")
    os.makedirs(image_dir, exist_ok=True)
    for i in range(1, num + 1):
        _write_tif(os.path.join(image_dir, f"MS_AOI_{AOI}_Moscow_chip{i}.tif"), count=8)

    if split == "train":
        mask_dir = os.path.join(aoi_root, "geojson_roads_speed")
        os.makedirs(mask_dir, exist_ok=True)
        for i in range(1, num + 1):
            _write_geojson(
                os.path.join(mask_dir, f"roads_speed_AOI_{AOI}_Moscow_chip{i}.geojson"),
                empty=(i % 2 == 1),
            )
    return str(tmp_path)
...
def test_getitem_train(tmp_path):
    ...
    ds = SpaceNet5(root=root, split="train", aois=[AOI], download=False)
    sample = ds[0]
...
def test_not_downloaded(tmp_path):
    with pytest.raises(DatasetNotFoundError):
        SpaceNet5(root=str(tmp_path), split="train", aois=[AOI], download=False)
