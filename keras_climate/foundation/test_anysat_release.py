...
from keras_climate.foundation.anysat_release import (
    AnySatRelease, anysat_projector_configs, ANYSAT_MODALITIES, ANYSAT_CONFIGS,
    pos_embed_with_scale, pos_embed_with_resolution, _bucket_ids_2d,
)
...
def test_pos_embed_shapes():
    ...
    pe_modis = pos_embed_with_scale(32, grid_size=4, scale=2, cls_token=True, modis=True)
...
def test_single_image_modality_builds_and_runs():
    model = AnySatRelease(["aerial"], {"aerial": (100, 100, 4)}, scale=1,
                           embed_dim=32, depth=1, num_heads=4, output="patch")
    out = model({"aerial": np.random.randn(2, 100, 100, 4).astype("float32")})
    assert tuple(out.shape) == (2, 2, 2, 32)


def test_tile_output_mode():
    model = AnySatRelease(["aerial"], {"aerial": (50, 50, 4)}, scale=1,
                           embed_dim=32, depth=1, num_heads=4, output="tile")
...
def test_time_series_modality_builds_and_runs():
    model = AnySatRelease(["s2"], {"s2": (5, 2, 2, 10)}, scale=2,
                           embed_dim=32, depth=1, num_heads=4)
    out = model({"s2": (np.random.randn(2, 5, 2, 2, 10).astype("float32"),
                         np.random.rand(2, 5).astype("float32") * 300)})
    assert tuple(out.shape) == (2, 1, 1, 32)


def test_mixed_image_and_time_series_modalities():
    model = AnySatRelease(["aerial", "s2"], {"aerial": (100, 100, 4), "s2": (5, 2, 2, 10)},
                           scale=1, embed_dim=32, depth=1, num_heads=4)
    out = model({
        "aerial": np.random.randn(2, 100, 100, 4).astype("float32"),
        "s2": (np.random.randn(2, 5, 2, 2, 10).astype("float32"), np.random.rand(2, 5).astype("float32") * 300),
    })
    assert tuple(out.shape) == (2, 2, 2, 32)
...
def test_inconsistent_footprint_raises():
    with pytest.raises(ValueError, match="patch grid"):
        AnySatRelease(["aerial", "s2"], {"aerial": (100, 100, 4), "s2": (5, 4, 4, 10)},
                       scale=1, embed_dim=32, depth=1, num_heads=4)
...
def test_weight_port_roundtrip_matches_pytorch_reference():
    ...
    class ImageProjector(nn.Module):
        def __init__(self, dim, patch_size, in_chans, resolution):
            super().__init__()
            self.patch_size, self.grid_size = patch_size, int(10 / resolution) // patch_size
            self.patch_embed = nn.Conv2d(in_chans, dim, patch_size, patch_size, bias=False)
            self.mlp = nn.Sequential(nn.Linear(dim, dim * 2), nn.LayerNorm(dim * 2), nn.ReLU(),
                                      nn.Linear(dim * 2, dim), nn.LayerNorm(dim), nn.ReLU())

        def forward(self, x, scale):
            t = self.patch_embed(x)
            gs = self.grid_size
            t = unfold(t, 2, gs); t = unfold(t, 3, gs)
            B, D, Ht, Wt = t.shape[:4]
            t = t.reshape(B, D, Ht, Wt, gs * gs)
            t = unfold(t, 2, scale); t = unfold(t, 3, scale)
            Htp, Wtp = Ht // scale, Wt // scale
            t = t.reshape(B, D, Htp * Wtp, gs * gs, scale, scale)
            t = t.permute(0, 1, 2, 4, 5, 3).reshape(B, D, Htp * Wtp, gs * gs * scale * scale)
    ...
    class LTAE2dCore(nn.Module):
        ...
        def __init__(self, dim, in_channels, n_head, d_k, T):
            ...
            self.inconv = nn.Sequential(*sum((
                [nn.Linear(dims[i], dims[i + 1]), nn.GroupNorm(4, dims[i + 1]), nn.ReLU(), nn.Dropout(0.0)]
            ...
            self.mlp = nn.Sequential(nn.Linear(self.d_model, self.d_model), nn.GroupNorm(4, self.d_model), nn.ReLU())
        ...
        def forward(self, x, dates):
            ...
            k = fc1_k(out).reshape(N, T, self.n_head, self.d_k).permute(2, 0, 1, 3).reshape(self.n_head * N, T, self.d_k)
            v = out.reshape(N, T, self.n_head, -1).permute(2, 0, 1, 3).reshape(self.n_head * N, T, -1)
            attn = F.softmax((q[:, None, :] @ k.transpose(1, 2)) / self.d_k ** 0.5, dim=-1)
    ...
    class TSProjector(nn.Module):
        ...
        def forward(self, x, dates, scale):
            ...
            o = unfold(o, 2, se); o = unfold(o, 3, se)
            Hp, Wp = H // se, W // se
            o = o.reshape(B, -1, Hp * Wp, se * se).permute(0, 2, 3, 1).reshape(B * Hp * Wp, se * se, -1)
            return o
    ...
    def sincos_scale(dim, grid_size, scale, cls_token=True):
        ...
        omega = torch.arange(dim // 4).float() / (dim // 4); omega = 1.0 / (10000 ** omega)
        def emb1d(pos):
    ...
    class CrossAttn(nn.Module):
        def __init__(self, dim, num_heads):
            super().__init__()
            self.num_heads, self.head_dim = num_heads, dim // num_heads
            self.wk, self.wv, self.proj = nn.Linear(dim, dim), nn.Linear(dim, dim), nn.Linear(dim, dim)
        ...
        def forward(self, h, pos_embed_q, n_modalities, bucket):
            ...
            q = q_.expand(B, -1, -1).reshape(B, Nq, self.num_heads, self.head_dim).transpose(1, 2)
            ...
            rpe_full = torch.cat([rpe[:, :, :, :1], rpe[:, :, :, 1:].repeat(1, 1, 1, n_modalities)], -1)
    ...
    class TorchAnySatRelease(nn.Module):
        ...
        def forward(self, x_aerial, x_s2, dates_s2, scale):
            ...
            tokens = torch.cat([cls, aerial_pooled + pe_global[1:], s2_pooled + pe_global[1:]], 1)

            for blk in self.blocks[:-1]:
                tokens = blk(tokens)

            bucket_cross, _ = bucket_ids(num_patches_side, num_patches_side, skip=1)
            tokens = self.blocks[-1](tokens, pe_global.unsqueeze(0), n_modalities=2, bucket=bucket_cross)
            return tokens[:, 1:].reshape(B, num_patches_side, num_patches_side, self.dim)
    ...
    keras_model = AnySatRelease(["aerial", "s2"], {"aerial": (img, img, 4), "s2": (T, 4, 4, 10)},
                                 scale, embed_dim=dim, depth=depth, num_heads=num_heads)
    keras_model({"aerial": np.zeros((B, img, img, 4), dtype="float32"),
                 "s2": (np.zeros((B, T, 4, 4, 10), dtype="float32"), np.zeros((B, T), dtype="float32"))})

    mapper = build_anysat_release_mapper(["aerial", "s2"], depth)
    report = WeightConverter(keras_model, state_dict, mapper, skip_patterns=[r"pad_parameter$"]).convert(
    ...
    out_k = keras.ops.convert_to_numpy(keras_model({
        "aerial": x_aerial_np, "s2": (x_s2_np, dates_s2.numpy().astype("float32"))}))

    max_diff = np.abs(out_t - out_k).max()
    assert max_diff < 1e-3, f"AnySatRelease weight port numerical mismatch: max abs diff {max_diff}"
