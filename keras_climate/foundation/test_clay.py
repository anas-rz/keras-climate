...
from keras_climate.foundation.clay import ClayEncoder, ClayClassifier, posemb_sincos_2d_with_gsd, posemb_sincos_1d
...
def test_encoder_builds_and_runs():
    encoder = ClayEncoder(img_size=64, patch_size=8, embed_dim=64, depth=2, num_heads=4,
                           dim_head=16, wave_dim=32, num_latent_tokens=8)
...
def test_classifier():
    ...
    clf = ClayClassifier(img_size=64, patch_size=8, embed_dim=32, depth=2, num_heads=4,
                          num_bands=6, num_classes=5)
...
def test_weight_port_roundtrip_matches_pytorch_reference():
    ...
    class WavesTransformer(nn.Module):
        def __init__(self, wave_dim, output_dim, num_latent_tokens, embed_dim, num_heads=4, num_layers=1):
            super().__init__()
            self.num_latent_tokens = num_latent_tokens
            layer = nn.TransformerEncoderLayer(d_model=wave_dim, nhead=num_heads, activation="gelu",
    ...
    class DynamicEmbedding(nn.Module):
        def __init__(self, wave_dim, num_latent_tokens, patch_size, embed_dim):
            ...
            self.weight_generator = WavesTransformer(wave_dim, patch_size * patch_size * embed_dim,
                                                       num_latent_tokens, embed_dim)
        ...
        def forward(self, batch, waves):
            ...
            dynamic_weight = weight.reshape(-1, self.embed_dim, self.patch_size,
                                             self.patch_size).permute(1, 0, 2, 3)
            out = F.conv2d(batch, dynamic_weight * 0.02, bias=bias * 0.02, stride=self.patch_size)
    ...
    class Attention(nn.Module):
        ...
        def forward(self, x):
            ...
            B, N, _ = x.shape
            q, k, v = [t.reshape(B, N, self.heads, self.dim_head).transpose(1, 2) for t in qkv]
    ...
    class Transformer(nn.Module):
        def __init__(self, dim, depth, heads, dim_head, mlp_dim):
            super().__init__()
            self.norm = nn.LayerNorm(dim)
            self.layers = nn.ModuleList([nn.ModuleList([Attention(dim, heads, dim_head), FeedForward(dim, mlp_dim)])
    ...
    class TorchClayEncoder(nn.Module):
        def __init__(self, img_size, patch_size, embed_dim, depth, num_heads, dim_head,
                     mlp_ratio, wave_dim, num_latent_tokens):
            super().__init__()
            self.embed_dim = embed_dim
            self.patch_size = patch_size
            ...
            self.transformer = Transformer(embed_dim, depth, num_heads, dim_head, int(embed_dim * mlp_ratio))

        def forward(self, pixels, waves, time_latlon, gsd=1.0):
            patches, _ = self.patch_embedding(pixels, waves)
            pos = torch_posemb_sincos_2d_with_gsd(self.grid, self.grid, self.embed_dim - 8, gsd=gsd).to(patches)
            B, L, _ = patches.shape
    ...
    torch.manual_seed(0)
    torch_model = TorchClayEncoder(IMG, PATCH, EMBED, DEPTH, HEADS, DIM_HEAD, 4.0, WAVE_DIM, NUM_LATENT)
    torch_model.eval()
    with torch.no_grad():
    ...
    translated = convert_clay_encoder_state_dict(flat, depth=DEPTH, num_latent_tokens=NUM_LATENT)

    keras_model = ClayEncoder(img_size=IMG, patch_size=PATCH, embed_dim=EMBED, depth=DEPTH,
                               num_heads=HEADS, dim_head=DIM_HEAD, wave_dim=WAVE_DIM,
    ...
    with torch.no_grad():
        ...
        torch_out = torch_model(torch_pixels, torch.from_numpy(waves), torch.from_numpy(time_latlon),
                                 gsd=gsd).numpy()

    keras_out = keras.ops.convert_to_numpy(
        keras_model({"pixels": pixels, "waves": waves, "time_latlon": time_latlon}, gsd=gsd, training=False))

    max_diff = np.abs(torch_out - keras_out).max()
    assert max_diff < 1e-3, f"Clay weight port numerical mismatch: max abs diff {max_diff}"
